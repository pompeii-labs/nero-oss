...
/** Concurrency cap for the worker fleet — the number of agents running at once. */
 ...
 *  ioredis vs ours — runtime-compatible (both ioredis 5.x), proven end-to-end. */
...
let flow: FlowProducer | null = null;
...
export function getFlowProducer(): FlowProducer {
    if (!flow) flow = new FlowProducer({ connection: luxRedis() });
    return flow;
}
