...
import { McpConnect } from '../services/mcp/connect';
...
export function mcpRoutes(): Hono {
    ...
    app.get('/v1/mcp/callback', async (c) => {
        try {
            ...
            if (errParam) return c.html(page('Authorization failed', errParam, false));
            if (!code || !state)
    ...
    app.post('/v1/mcp/reconnect', async (c) => {
        try {
            ...
            if (!body.name) return error(c, 400, 'name required');
    ...
    app.post('/v1/mcp/connect', async (c) => {
        try {
            ...
            if (!body.name || !body.url) return error(c, 400, 'name and url required');
            const r = await McpConnect.start({
                name: body.name,
                url: body.url,
                apiKey: body.apiKey,
            });
