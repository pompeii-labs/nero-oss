...
import { Panel, type OpenPanelInput } from '../src/models/panel';
...
async function make(extra: Partial<OpenPanelInput> = {}) {
...
d('panels data layer (Lux)', () => {
    ...
    test('update geometry persists', async () => {
        const p = await make();
        await Panel.update(p.id, { x: 111, y: 222, w: 333, h: 444 });
        const got = await Panel.get(p.id);
    ...
    test('maximize flag round-trips', async () => {
        const p = await make();
        await Panel.update(p.id, { maximized: true });
        expect((await Panel.get(p.id))?.maximized).toBe(true);
        await Panel.update(p.id, { maximized: false });
        expect((await Panel.get(p.id))?.maximized).toBe(false);
    });

    test('listOpen returns open panels, close removes them', async () => {
        const p = await make();
        let open = await Panel.listOpen(DEVICE);
        expect(open.some((x) => x.id === p.id)).toBe(true);
        await Panel.close(p.id);
        open = await Panel.listOpen(DEVICE);
